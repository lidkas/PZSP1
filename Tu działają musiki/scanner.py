...
def receive_and_guess(ard_conn, models, wait_button=False):
    file_name = 'curr_card.bmp'
    ard_conn.get_and_save_image(file_name, wait_button)
    color_model, value_model = models
    color = color_dictionary[guess(file_name, color_model)]
    value = value_dictionary[guess(file_name, value_model)]
    return color, value
